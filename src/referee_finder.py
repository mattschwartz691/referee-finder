...
from .models import Paper, Author, RefereeCandidate
...
class RefereeFinder:
    ...
    def find_referees(
        ...
        """
        Find suitable referee candidates for a paper.

        Args:
            arxiv_id: arXiv identifier of the paper
            num_candidates: Number of candidates to return
            months_start: Minimum months ago for related papers
            months_end: Maximum months ago for related papers

        Returns:
            List of RefereeCandidate objects, ranked by relevance
        """
        ...
        # Sort by number of relevant papers (most promising first)
        ...
        for author_name, relevant_papers in sorted_authors:
            if len(candidates) >= num_candidates * 2:
                # We have enough candidates, stop evaluating
                break
            ...
            if not self.inspire.is_active(author_name):
    ...
    def _is_paper_author(self, candidate: str, paper_authors: List[str]) -> bool:
        """Check if candidate is one of the paper authors."""
    ...
    def format_results(self, candidates: List[RefereeCandidate], arxiv_id: str) -> str:
        ...
        for i, candidate in enumerate(candidates, 1):
            ...
            if author.career_stage and author.career_years:
                lines.append(f"   Career Stage: {author.career_stage} ({author.career_years} years)")
